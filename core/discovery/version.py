...
设计（与 EcsCloud 对齐）：
  * 每个版本独立维护一套脚本：projects/<id>/<version>/api/<模块>_API测试.py
...
def scripts_dir_for(project_dir, version: Optional[str] = None) -> Path:
    ...
    新结构: projects/<id>/<version>/  （扁平化，不再嵌套 scripts/）
    """
    project_dir = Path(project_dir)
    version = version or resolve_version(project_dir)
    d = project_dir / version
...
def available_versions(project_dir) -> list:
    ...
    project_dir = Path(project_dir)
    # 优先查找版本目录（v* 开头）
    versions = [d.name for d in project_dir.iterdir()
