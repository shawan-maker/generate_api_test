...
def _load_playbook(project_dir: Path, module_name: str) -> dict | None:
    """从 workspace_dir/kb/module_discovered/{module_name}_playbook.json 加载 playbook"""
    if not project_dir or not module_name:
        return None

    workspace_dir = get_workspace_dir(project_dir)
    playbook_path = workspace_dir / "kb" / "module_discovered" / f"{module_name}_playbook.json"
