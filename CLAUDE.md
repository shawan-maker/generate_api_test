...
```bash
# 主管线（Stage 1-5）
python -m core.discovery.run --project <id> --stage <1|2|34|4|5|all>

# 批量处理
python -m core.discovery.run --project <id> --all-modules --stage <stage>

# 并行测试执行
python -m core.discovery.run_parallel --project <id>
...
# 独立登录工具（从项目目录执行）
cd projects/<project>
PYTHONPATH=../.. python -m lib.auth.login_tool --output <version>/ui/config/cookies.json

# P0 爬虫
python -m core.crawler.playwright_crawl --project <id>
```

## 输出规范

- **运行时输出** 统一在 `projects/<project>/output/`（logs、config、debug）
- **版本化脚本包** 在 `projects/<project>/<version>/`（api/、ui/、export/ 子目录）
- **KB 中间产物** 在 `projects/<project>/kb/module_discovered/`（manifest、capture、playbook）
- **Stage 5 导出** 在 `projects/<project>/<version>/export/<module>/`（Postman、helpers）
- 根目录不产生任何运行时输出
...
- **运行时同步**：gen_test.py / generate_ui_script.py 自动同步 lib/ 到脚本包（含导入路径转换）
- **UI 通知钉住**：MutationObserver 检测 el-message/el-notification 后钉住元素，截图后在 _cleanup_dialogs 释放
