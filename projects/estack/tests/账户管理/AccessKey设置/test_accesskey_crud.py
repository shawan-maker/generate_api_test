"""
test_accesskey_crud.py — accesskey 回归用例: 创建→查询(列表/详情)→删除→校验消失。
...
list_ = get_fn(_PROJ, _SVC, _RES, "list") if True else None
detail = get_fn(_PROJ, _SVC, _RES, "detail") if False else None
...
class TestAccesskeyCRUD:

    def test_Accesskey_create(self, client):
        """create: POST /estack/api/estack/draco/v1/accesskey/create"""
        if create is None:
    ...
    def test_Accesskey_list(self, client):
    ...
    def test_Accesskey_detail(self, client):
        """detail: """
        if detail is None:
    ...
    def test_Accesskey_delete(self, client):
        """delete: """
        if delete is None:
