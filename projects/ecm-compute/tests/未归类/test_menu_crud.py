"""
test_menu_crud.py — menu 回归用例: 创建→查询(列表/详情)→删除→校验消失。
...
class TestMenuCRUD:
    ...
    def test_Menu_create(self, client):
        """create: POST /estack/api/estack/pegasi/v1/menu/side-tree"""
        if create is None:
    ...
    def test_Menu_detail(self, client):
        """detail: """
        if detail is None:
    ...
    def test_Menu_delete(self, client):
        """delete: """
        if delete is None:
