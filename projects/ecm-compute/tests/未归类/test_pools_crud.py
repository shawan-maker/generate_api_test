"""
test_pools_crud.py — pools 回归用例: 创建→查询(列表/详情)→删除→校验消失。
...
class TestPoolsCRUD:
    ...
    def test_Pools_create(self, client):
        """create: POST /estack/api/estack/cancer/inner/v1/pools/list/with-region/public"""
        if create is None:
    ...
    def test_Pools_list(self, client):
    ...
    def test_Pools_detail(self, client):
        """detail: """
        if detail is None:
    ...
    def test_Pools_delete(self, client):
        """delete: """
        if delete is None:
